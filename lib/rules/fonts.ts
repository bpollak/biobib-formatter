  ...
  },
  {
    ...
    check(doc: DocumentModel): RuleResult {
      ...
      if (outOfRange.length === 0) {
        return pass('FONT-002', 'fonts', 'Font Size 10–12pt', 'critical', true,
      ...
      return fail('FONT-002', 'fonts', 'Font Size 10–12pt', 'critical', true,
  ...
  },
  {
    ...
    check(doc: DocumentModel): RuleResult {
      ...
      if (uniqueFonts.length <= 2) { // Allow some variation (e.g., normal + monospace for code)
        return pass('FONT-003', 'fonts', 'Consistent Font Throughout', 'major', true,
          `Font is consistent: ${uniqueFonts.join(', ')}`);
      }
      return fail('FONT-003', 'fonts', 'Consistent Font Throughout', 'major', true,
  ...
  },
  {
    ...
    check(doc: DocumentModel): RuleResult {
      ...
      if (uniqueSizes.length <= 1) {
        return pass('FONT-004', 'fonts', 'Consistent Font Size', 'major', true,
          `Font size is consistent: ${uniqueSizes.map(s => `${s/2}pt`).join(', ')}`);
      }
      return fail('FONT-004', 'fonts', 'Consistent Font Size', 'major', true,
  ...
  },
  {
    ...
    check(doc: DocumentModel): RuleResult {
      ...
      if (blueColors.length === 0) {
        return pass('FONT-006', 'fonts', 'No Colored Hyperlinks', 'major', true,
          'No colored hyperlinks detected');
      }
      return fail('FONT-006', 'fonts', 'No Colored Hyperlinks', 'major', true,
