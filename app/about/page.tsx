...
const CURRENT_LIMITATIONS = [
  ...
  'The original uploaded CV file and parsed source text are removed after processing. Temporary job status, section results, and the generated BioBib remain available for recovery and download; automatic retention cleanup is not yet available.',
...
const FAQ_ITEMS = [
  ...
  },
  {
    ...
    a: 'The original uploaded CV file is removed after the app reads it, and the parsed source text is removed when conversion finishes. Temporary section results, status information, and the generated BioBib remain available for progress recovery and download.',
  },
  {
    ...
    <Container maxWidth={false} sx={{ maxWidth: PAGE_MAX_WIDTH, py: 6 }}>
      <Box sx={{ mb: 5 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mt: 2 }}>
          ...
          <Chip label="23 review parts" size="small" color="primary" />
      ...
      <Box sx={{ mb: 5 }}>
        ...
        <Box sx={{ mb: 2.5, pl: 2, borderLeft: '3px solid #C69214' }}>
          <Typography variant="body1" fontWeight={700} sx={{ mb: 0.5 }}>
            {RELEASE_NOTES[0].title}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Released {RELEASE_NOTES[0].releasedAt}
          </Typography>
          <Box component="ul" sx={{ pl: 3, mt: 1.25, mb: 0 }}>
            {RELEASE_NOTES[0].changes.map((change) => (
        ...
        <Link
          href="/release-notes"
          style={{ color: '#00629B', fontWeight: 700, textDecoration: 'underline' }}
        >
